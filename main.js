...
async function createSelectPlayerTable(availablePlayers, flag) {
    ...
    const selectPlayerTable = $('#selectPlayerTable').DataTable({
        ...
        columnDefs: [
            ...
            { targets: [2], title: "ADP", className: "dt-center", type: "adp-sort", visible: false },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.test').forEach(button => {
        button.addEventListener('click', () => {
            ...
            numTeams = 8;
            userDraftPosition = Math.floor(Math.random() * 8) + 1;
            draftType = 'snake';
            document.getElementById('userDraftPosition').value = Math.floor(Math.random() * 8) + 1;
